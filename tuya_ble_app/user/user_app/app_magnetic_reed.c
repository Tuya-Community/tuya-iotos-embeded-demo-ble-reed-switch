...
void app_ctrl_handle(void)
{
    ...
    if(gpio_read(SWITCH_IO) == 0) {
        app_ctrl_data.switch_state = 0;
    }else {
    	app_ctrl_data.switch_state = 1;
    }
    dp_value = app_ctrl_data.switch_state;
    app_dp_report(DP_SWITCH_STATE, &dp_value, Bool);
